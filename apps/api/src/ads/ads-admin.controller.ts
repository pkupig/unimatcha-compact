...
export class AdsAdminController {
  ...
  listCampaigns(
    ...
    return this.adsService.listCampaigns(admin, { status, schoolId, page, limit });
  }
  ...
  @ApiQuery({ name: 'from', required: false, description: '起始日期（含），ISO' })
  @ApiQuery({ name: 'to', required: false, description: '截止日期（含），ISO' })
  getCampaignStats(
    @CurrentUser() admin: CurrentAdmin,
    @Param('id') id: string,
    ...
    return this.adsService.getCampaignStats(admin, id, from, to);
  }
