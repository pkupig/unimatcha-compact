...
/**
 * 跨校合作消息线程（B6）：商家 × 学校洽谈通道。
 * 目录互见（商家看学校 / 学生会看他校商家）+ 唯一线程收发 + 未读角标；
...
export class PartnersController {
  ...
  @ApiOperation({ summary: '学校目录（商家侧）：启用中且有活跃学生会入驻，附既有线程 id' })
  ...
  @ApiOperation({ summary: '商家目录（学生会侧）：他校自拉的活跃商家，不含联系方式' })
  ...
  @ApiOperation({ summary: '发起洽谈（同一商家×学校唯一线程，重复发起并入并附 existed:true）' })
  ...
  @ApiOperation({ summary: '线程列表（商家=本人 / 学生会=本校 / 平台=全量只读），最新消息在前' })
