...
| P1-11 | **校标后台维护页**：`School.badgeUrl/badgeText/badgeColor` 三列与 `GET /public/school-badges` 已就位，admin-web 缺一个编辑表单 | 现在校标全走前端自动生成（取校名首字母 + 按校名哈希配色），认不出真校徽 | 用户拍板的口径是「先自动生成 + 后台可覆盖」，自动那半已上线；后台这半在有真实校徽素材时补。**注意**：校标只认 `User.verifiedSchool`（管理员审核通过时写的快照），不要改成读 `Profile.school`——那是用户可改的下拉框，改一下就能挂任意学校的校标 |
| P1-12 | **聊天消息翻译**：长按菜单已留出位置，接一个翻译服务即可 | 中英混合校园场景下有真实需求 | 用户 2026-09-05 拍板**本轮不做**，只保留入口。接的时候注意：消息正文是用户内容，译文要与原文并排显示而不是替换，且不能进全局词典 |
...
| P2-6 | **离线推送**（H5 Web Push：SW+VAPID，自签发零外部账号；iOS 上架后 APNs） | SSE 已覆盖「App 打开时」的实时（2026-08-31）；用户要「关着 App 也收到」时做。SW 只注册 push 事件、**绝不加 fetch 缓存**（7/26 启动页缓存事故的教训） |
| P2-7 | **uploads 迁对象存储 + CDN**：S3 兼容（DO Spaces/R2）+ 改 /uploads/image 返回对象 URL | 单机磁盘/带宽成瓶颈时；已具备的缓解：每日备份含 uploads 卷、uuid 文件名已加一年 immutable 强缓存（2026-08-31）。需用户开服务商账号，可与异地备份共用 |
| P2-8 | **架构拆分**：DB 独立（DO Managed PG）→ api 多实例（SSE 需换 Redis pub/sub，RealtimeService 已注释好切换点）→ 读写分离 | 单机 CPU/内存 >70% 持续或 DAU 上千；在那之前纵向升配 + 备份是正解 |
| P2-9 | **客服工单 / EDM 触达**：反馈系统已有（/feedback），EDM 直接用 Brevo（已接入，营销功能自带） | 有真实运营需求时 |
